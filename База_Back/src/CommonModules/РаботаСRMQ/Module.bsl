...
Функция ВыполнитьОтправкуСообщенийПоНастройкамПодключения(НастройкаПодключения = Неопределено) Экспорт
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПодключенияКОчереди КАКПодключенияКОчереди
	|		ПО СообщенияКОтправке.КлючМаршрутизации = КАКПодключенияКОчереди.КлючМаршрутизации
	|		И КАКПодключенияКОчереди.Направление = &Направление
	|ГДЕ
	...
	Запрос.УстановитьПараметр("Направление", Перечисления.отусНаправления.Отправление);
...
Функция ПрочитатьJSONОбъекта(ИмяОчереди, ПолученноеСообщение)  Экспорт    
	...
	Если ЭтоОбъект Тогда
		...
		Если СтрНайти(Строка(ТипЗнч(Данные)), "Документ") <> 0 Тогда
			Данные.Записать(РежимЗаписиДокумента.Проведение);
		Иначе	
			Данные.Записать();
		КонецЕсли;	
	Иначе
		Данные = ПрочитатьJSONРегистра(ИмяОчереди, ПолученноеСообщение, Чтение);
	КонецЕсли;	
...
Функция ПрочитатьJSONРегистра(ИмяОчереди, ПолученноеСообщение, Чтение) 
...
Функция ВыполнитьПолучениеСообщенийПоНастройкамПодключения(НастройкаПодключения = Неопределено) Экспорт  
	...
	Запрос.Текст = 
		...
		|	НастройкиПодключенияRMQОчереди.Порядок КАК Порядок
		|ИЗ
		...
		|	И НастройкиПодключенияRMQОчереди.Направление = &Получение
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок";
	
	Запрос.УстановитьПараметр("НастройкаПодключения", мПодключений);
	Запрос.УстановитьПараметр("Получение", Перечисления.отусНаправления.Получение);
