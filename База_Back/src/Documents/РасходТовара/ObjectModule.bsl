...
Процедура ОбработкаПроведения(Отказ, Режим)   
	...
	ПроцентБонусов = Константы.отусПроцентНачисленияБонусов.Получить();  
	
	// Создадим запрос, чтобы получать информацию об услугах
	Запрос = Новый Запрос("ВЫБРАТЬ
	|    ТоварыВДокументе.НомерСтроки КАК НомерСтроки,
	|		ВЫБОР КОГДА ТоварыВДокументе.Товар.Наименование = ""Подарочный сертификат""
	|			ТОГДА Истина ИНАЧЕ ЛОЖЬ КОНЕЦ  КАК ЭтоПодарочныйСертификат
	|ИЗ
	...
	Если ЭтоФронт  Тогда 
		...
		Для каждого ТекСтрокаТовары Из Товары Цикл    
			...
			Если Строка = Неопределено Тогда
				Если ЗначениеЗаполнено(отусВиртуальнаяКарта) И ЗначениеЗаполнено(ПроцентБонусов) Тогда   
					Движения.отусБалансВиртуальныхКарт.Записывать = Истина;
					Движение = Движения.отусБалансВиртуальныхКарт.Добавить();
					Движение.ВидДвижения = ВидДвиженияНакопления.Приход;
					Движение.Период = Дата;
					Движение.Карта = отусВиртуальнаяКарта;
					Движение.Сумма = Товары.Итог("Сумма") * ПроцентБонусов / 100; 
				КонецЕсли;
			Иначе	
				Если Строка.ЭтоПодарочныйСертификат Тогда					
					Движения.отусБалансВиртуальныхКарт.Записывать = Истина;
					Движение = Движения.отусБалансВиртуальныхКарт.Добавить();
					Движение.ВидДвижения = ВидДвиженияНакопления.Приход;
					Движение.Период = Дата;
	...
	Если Не ЭтоФронт Тогда
		...
		// otus project >>> 
		// Формирование движения регистра накопления отусБалансВиртуальныхКарт.
		
		ПодарочныйСертификат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(отусВиртуальнаяКарта, "ПодарочныйСертификат");
		
		Если ЗначениеЗаполнено(отусВиртуальнаяКарта) И ЗначениеЗаполнено(ПроцентБонусов)
			И Не ПодарочныйСертификат Тогда   
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Оплата.Количество() = 0 Тогда
		...
		НоваяОплата.СпособОплаты = Справочники.отусСпособыОплат.Наличные;
		НоваяОплата.Сумма = Товары.Итог("Сумма");
